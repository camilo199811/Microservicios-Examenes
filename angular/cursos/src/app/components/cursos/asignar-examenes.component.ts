...
export class AsignarExamenesComponent implements OnInit {
  ...
  mostrarColumnasAlumnos=['id','nombre','asignaturas','eliminar']
  examnes:Examen[]=[];
  ...
  ngOnInit(): void {
    this.route.paramMap.subscribe(params => {
      ...
      this.cursoService.ver(id).subscribe(c => {this.curso = c; this.examnes=this.curso.examenes;this.iniciarPaginador()});

    });
    this.autocompleteControl.valueChanges.pipe(
      map(valor => typeof valor === 'string' ? valor : valor.nombre),
      flatMap(valor => valor ? this.examenService.flitrarPorNombre(valor) : [])
    ).subscribe(examenes => this.examenesFiltrados = examenes);
  }

 private iniciarPaginador(){
    this.datasource=new MatTableDataSource<Examen>(this.examnes);
  ...
  seleccionarExamen(event: MatAutocompleteSelectedEvent): void {
    ...
    if (!this.existe(examen.id)) {
      ...
      Swal.fire('Error',
        `El examen ${examen.nombre} ya esta asignado al curso`, 'error');
    }
    this.autocompleteControl.setValue('');
    event.option.deselect();
    event.option.focus();
  }
  //ver si el examen ya ha sido asignado al curso
  private existe(id: number): boolean {
    let existe = false;
    this.examenesAsignar.concat(this.curso.examenes).forEach(
      e => {
        if (id === e.id) {
          existe = true;
        }
  ...
  asignar(): void {
    ...
    this.cursoService.asignarExamenes(this.curso, this.examenesAsignar)
      .subscribe(curso => {
        this.examnes=this.examnes.concat(this.examenesAsignar);
        this.iniciarPaginador();
        this.examenesAsignar = [];

        Swal.fire('Asignados:',
          `Examenes asignado con exito al curso ${this.curso.nombre}`,
  ...
  eliminarExamenDelCurso(examen:Examen):void{
    ...
    .subscribe(curso=>{
      this.examnes=this.examnes.filter(e=>e.id !== examen.id);
      this.iniciarPaginador();
